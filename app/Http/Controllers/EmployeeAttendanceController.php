...
class EmployeeAttendanceController extends Controller
{
    function attendanceList(Request $req){
        $data = Employee::getInstance()->where('email',session('user'))->get(); 
        $list = EmployeeAttendance::getInstance()->where('employee_id',$data[0]->id)->orderBy("id","Desc")->get();
    ...
    function inTime(Request $req){
        ...
        $data = Employee::getInstance()->where('email',session('user'))->get(); 
        $attendance = EmployeeAttendance::getInstance();
    ...
    function leaveTime(Request $req){
        date_default_timezone_set("Asia/Dhaka");
        $attendance =EmployeeAttendance::getInstance()->find($req->id);
    ...
    function show(Request $req){
        ...
        $data = Employee::getInstance()->where('email',session('user'))->get(); 
        // dd($data[0]->id);
        $list = EmployeeAttendance::getInstance()->where('enter_date', date("Y-m-d"))->where('employee_id',226)->get();
        $list['chart'] = EmployeeAttendance::getInstance()->where("employee_id",$data[0]->id )->selectRaw('
    ...
    function adminReport(Request $req){
        ...
        $report = EmployeeAttendance::getInstance()->with('employee')->selectRaw('
        COUNT(*) as working,
        MONTHNAME(enter_date) as month,
    ...
    function report(Request $req){
        $data = Employee::getInstance()->where('email',session('user'))->get(); 
        $report = EmployeeAttendance::getInstance()->where("employee_id",$data[0]->id )->selectRaw('
    ...
    function showAttendanceList(Request $req){
        $list = EmployeeAttendance::getInstance()->with("employee")->orderBy('id',"desc")->get();
        // return $datos;
